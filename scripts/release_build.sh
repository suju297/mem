...
if [[ -z "$VERSION" ]] && command -v python3 >/dev/null 2>&1 && [[ -f "$ROOT_DIR/extensions/vscode-mem/package.json" ]]; then
  VERSION="v$(python3 - <<'PY'
import json, os
path = os.path.join(os.environ["ROOT_DIR"], "extensions", "vscode-mem", "package.json")
...
for target in "${targets[@]}"; do
  ...
  CGO_ENABLED=0 GOOS="$os" GOARCH="$arch" \
    go build -trimpath -ldflags "-s -w -X mem/internal/app.Version=$VERSION -X mem/internal/app.Commit=$COMMIT" \
