...
func writeUsage(w io.Writer) {
	...
	io.WriteString(w, "  threads         mem threads [--workspace <name>] [--repo <id>]\n")
	io.WriteString(w, "  thread          mem thread <thread_id> [--limit 20] [--workspace <name>] [--repo <id>]\n")
	io.WriteString(w, "  mcp             mem mcp [start|stop|status] [--repo <id|path>] [--allow-write] [--write-mode ask|auto|off] [--debug] [--repair]\n")
