...
func runShareExport(args []string, out, errOut io.Writer) int {
	...
	outDir := fs.String("out", "", "Output directory (default: mem-share in repo root)")
...
func runShareImport(args []string, out, errOut io.Writer) int {
	...
	repoOverride := fs.String("repo", "", "Override repo id/path")
	workspace := fs.String("workspace", "", "Workspace name")
	inDir := fs.String("in", "", "Input directory (default: mem-share in repo root)")
...
This folder is generated by Mem.
...
Mem will ask for confirmation before importing across repo ids.
