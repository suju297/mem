...
This page provides the full command grammar in a Docopt-style format.
...
mem [--data-dir <path>] mcp (start | stop | status) [--repo <id|path>] [--require-repo] [--allow-write] [--write-mode ask|auto|off] [--debug] [--repair]
...
- Use `mem <command> --help` for command-specific details.
- For machine-readable output, use `--format json` where supported.
